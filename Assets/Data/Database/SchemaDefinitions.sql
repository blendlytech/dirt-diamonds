...
-- Schema version 4 — adds Pitcher_Roles + Pitch_Arsenals with idempotent
-- backfills (all additive; IF NOT EXISTS + INSERT OR IGNORE migrate v3 saves
-- in place on boot, same pattern as v2→v3). Reliever Players rows for
-- migrated saves are generated by LeagueGenerator.EnsureV4 at boot — DDL
-- cannot invent people. Bump with every migration.
