...
/// simulation state; the driver owns every write).
/// </summary>
public readonly struct AtBatViewState
{
    ...
    public AtBatViewState(
        ...
        int balls, int strikes, int outs, int bases)
    {
        AwayScore = awayScore;
        HomeScore = homeScore;
        Inning = inning;
...
{
    /// <summary>Player committed a swing; timing error is the slider's τ ∈ [-1, +1] (§6).</summary>
    [Signal]
    public delegate void SwingCommittedEventHandler(double timingError);

    /// <summary>Player took the pitch.</summary>
    [Signal]
    public delegate void TakeCommittedEventHandler();
    ...
    private void OnSwingPressed()
    {
        ...
        EmitSignal(SignalName.SwingCommitted, _timingSlider.Value);
    }

    private void OnTakePressed()
    {
        SetIntentEnabled(false);
        EmitSignal(SignalName.TakeCommitted);
    }
