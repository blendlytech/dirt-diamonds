...
**Field/op/consequence vocabulary** (loader-enforced, listed here for authoring): fields `funds|age|recklessness|health_ceiling|detection_risk|baseball_interest`; ops `< <= > >= == !=`; consequences `funds|stress|interest|set_flag|clear_flag|relationship` (relationship: `kind` rival/friend/partner, `target` teammate/opponent/league). `min_days_since` on `flag_active` is the cascade clock.