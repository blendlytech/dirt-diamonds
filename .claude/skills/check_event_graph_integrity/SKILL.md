...
**Field/op/consequence vocabulary** (loader-enforced, listed here for authoring): fields `funds|age|recklessness|health_ceiling|detection_risk|baseball_interest`; ops `< <= > >= == !=`; consequences `funds|stress|interest|detection_risk|health_ceiling|set_flag|clear_flag|relationship|conceive_child|absence` (relationship: `kind` rival/friend/partner, `target` teammate/opponent/league; `absence`: `{"type":"absence","reason":"injury"|"suspension"|"arrest","days":N≥1}`). `min_days_since` on `flag_active` is the cascade clock.
