...
Fresh save, either avatar from Session A. Let days tick (autopilot or light play) from day 2. This replaces the old "expect zero week-1 events" expectation — that was the bug (a fresh freshman got nothing narratively for its opening weeks); the onboarding arc is the fix. (An existing pre-fix save works too: the avatar had provably never received ANY event — see the dispatcher starvation fix in the Stage-3 review entry — so `hs_first_day` arrives on the first day-advance after this build, whatever the calendar says.) The retired day-1 phone tutorial's one-day shift is gone: `hs_first_day` is back on **day 2**.

- [ ] **Days 2–14 deliver the onboarding beats on the BurnerPhone in roughly chain order:** first day (Mom) → meet the coach → tryouts → first practice → a lunchroom/social beat (a classmate, "Marcus") → first-game nerves. Roughly one beat per day, tapering off after about day 9. (A `hs_crush_forms` dating-funnel text may interleave on some days — that's the documented ~5%/day romance overlap, not a bug; it never drops an onboarding beat, only occasionally shifts one by a day.)
...
## Session A2b — Onboarding tutorial overlay + self-explaining needs UI (`docs/design/onboarding_tutorial_overlay.md`)

The day-1 phone tutorial events (`tut_*`, the "Getting Started" thread) are **retired** — replaced by a day-1 UI overlay plus permanent needs-UI upgrades. **No Getting-Started card should ever appear** (its contact row survives as a dormant hint-thread hook, which is fine — an empty thread, not a firing one). The `TutorialOverlay` (T-2) is built and reviewed — every line below is live.

- [ ] **Fresh save → the overlay opens on day 1**; all 8 steps advance, Back works, Skip works.
- [ ] **While the overlay is up, the Time bar reads "Paused — tutorial open"** and the clock face doesn't move; closing (or skipping) the tutorial lets time flow again at the speed you'd chosen.
- [ ] **The overlay is click-through:** with a step up you can still tap phone tabs (step 3 says "Open Bank" — do it, and the spotlight moves onto the Needs card) and the card's own Next/Back/Skip buttons still work.
