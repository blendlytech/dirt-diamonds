...
- **`DIRT-3` — "Product & Turf."** **→ NOW FULLY SPECCED: `docs/design/criminal_underworld_dirt3_product_and_turf.md` (Opus, 2026-07-13).** Reactive narrative wrap around the shipped `HustleService` engine flags — gate on them (`flag_active`, pre-satisfied per check #2) so the interactive hustle mini-games and the narrative layer reference the same state. **NB: the shipped turf flag is `controls_turf_local` (not the outline's stale `controls_turf_f`/`controls_turf_<n>`), and four newer flags now exist (`hot_goods`, `robbery_bust`, `spoiled_goods`, `gambling_bust`) — the full spec verifies all seven against `HustleService.cs` and requires a one-line check #2 allowlist correction.** A bad batch that comes back, a contested corner (injury teeth), the robbery arrest-absence teeth deferred from `hustle_minigames_depth_pass.md` §11, and the robbery→fence economy nudge. Sets `in_the_life` — hustling becomes the arc's **third on-ramp**.
