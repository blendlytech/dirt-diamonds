...
- **`DIRT-2` — "The Fix" (tier ≥ 2, the diamonds-meet-dirt centerpiece).** Entry gates on `flag_active in_the_life` **and** `tier >= 2` (rostered pro). The syndicate calls in the marker: shave runs, tip a line, throw a game. Stakes are baseball-terminal — a `suspension` absence, a permanent `rival` edge with `target: league`, a `banned_for_life` flag that later batches (and the career manager, if wired) can read. The clean, high-value use of the shipped syndicate spine: `compromised_syndicate` is the *prerequisite*, the fix is the *ask*.
