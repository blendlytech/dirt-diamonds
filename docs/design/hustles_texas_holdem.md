...
## 16. Fable 5 review verdicts (2026-07-05 addendum — the §15 review pass)

Reviewed at HEAD `f40b09e`. All harnesses green at review time: `HoldemHarness` 63/63, `GrittyEventsHarness` 67/67, `run_monte_carlo_batch` 167/167 with the MLB bit-identity guard exact — no baseball band moved.

1. **§10 state-representation: the pooled-mutable-class fallback is UPHELD** (not merely tolerated). The decisive argument: the §8.2 interactive contract has the UI *retain* `session.Table` across frames between hero decisions, so the state's natural home is a stable reference — the `[InlineArray]` struct recommendation dissolves at exactly the seam the state crosses. Allocation is once per session, `ResetHand` reuses everything, and the harness pins warm hands at ~0 B, which is what the zero-GC mandate actually demands. Do not "upgrade" this to the struct pattern later; the recommendation half of §10 is superseded by this ruling.
2. **Writers APPROVED.** No new SQL writer exists; everything routes through 8b's atomic clamped writers via the shared `ApplyCore`, `gambling_bust` carries `set_on_day` (the 8c cascade clock), stress publishes post-commit, and `ToResolution` hard-codes `healthCeilingDelta: 0` — §11's deferral held.
3. **Chip conservation APPROVED, analytically.** The §8.3 level-sum identity Σlayers = Pot holds exactly (folded dead money included), the progressive rake drain therefore always collects the full nominal rake, and the empty-eligible-layer division-by-zero is structurally impossible (at showdown the max-committed seat is never folded; the all-folded case exits via the fold path). The 8d-2 multiway-all-in rake fix is correct.
4. **The `c/P` pot-odds implementation is CONFIRMED as this doc's intended math — §6.1's `c/(P+c)` formula line is the error, not the code.** With P defined (as §6.1's own prose defines it) to include the bet being faced, `c/P = s/(1+s)`, matching §6.1's worked examples and §6.2's "two views of the same line" exactly; `c/(P+c)` is the different quantity `s/(1+2s)`. Self-consistency between the call threshold and §6.3's bluff-ratio is the load-bearing property (equity estimates are vs-random, i.e. range-blind, so the tighter threshold is also directionally right). **Any future change to `HoldemAgent.RequiredEquity` must re-run the check-6 EV bands before merging.**
5. **Skill bands APPROVED.** Measured (Low, 60k hands/policy, common random numbers): TAG 214.60 > LAG 128.41 > Nit 75.61 ≈ Station 70.39 ≫ Maniac −500.94 ≫ Random −1523.95 bb/100; TAG at High 116.56 < Low. The two disclosed divergences are both accepted, no retune: the homogeneous-all-TAG-field ranking flip (nothing in v1 exploits a tight range's predictability — §13's ordering is a claim about the tier's real field mix, which is what the harness asserts) and §13's "calling-station hero: negative" table row (a Station in a Maniac-heavy Low field is passively executing §6.3's own prescribed exploit against over-bluffers; its positive win-rate there is the model agreeing with its own theory — the row's prediction holds against disciplined fields, not soft ones).
6. **Minor, disclosed-not-actioned:** rake is taken on the uncalled portion of a final bet (real rooms return it first; bounded by the 3·BB cap — v2 nicety); a below-entry-threshold hand never defends the BB on pot odds (VPIP-chart simplification, priced into the bands); `GetFieldSkew` exposes a shared mutable array (internal-only; `ReadOnlySpan<double>` when next touched); `Assets/Economy/Hustles/TexasHoldemLogic.cs` is an empty Phase-0 scaffold leftover to delete in a future cleanup.
