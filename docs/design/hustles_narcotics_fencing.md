...
## 12. Fable 5 addendum (2026-07-05, 8c pass) — the §3.4 re-band watch-item, closed

§11's watch-item said to re-band §3.4 "once the tail actually costs something." 8c shipped that machinery (see `roster_availability_triad.md`): the `absence` consequence + `Player_Absences` + the `AvailabilityLedger` mean an event keyed on `detection_risk` / `health_ceiling` / `narc_watchlist` can now bench a player for real games. Rulings:

- **§3.4's normative resolver-level bands are the measured figures: Safe +$93.9, Moderate +$225.5, Aggressive +$1,444.2** (20k seeded runs each, closed-form-verified, `HustleHarness`-asserted). These are *pre-career-cost* numbers and stay the harness contract — the resolver's constants were deliberately not touched in 8c.
- **The tail's price is a content knob now, not a resolver knob.** One absent day = one missed game in-season; an **arrest** day additionally locks the daily schedule (≈$200/day of blocked Legal Work at 8a's rate, plus no hustle sessions); a **suspension** leaves the schedule free (its price is games/stats — weakly monetized until 9c's promotion gates read season lines); an **injury** benches *and* docks post-return ratings automatically off live `health_ceiling` (engine-computed rust, max −12).
- **Costing frame handed to Sonnet's 8c content:** suspension 5–15 days at tiered `detection_risk` thresholds (≥60 / ≥80 / ≥90), arrest 20–40 days keyed `narc_watchlist` (+ high detection), injury 7–21 days keyed `health_ceiling < 50`. At those bands, Aggressive's ≈18% catastrophic tail realizes roughly $700–$1,100 of expected career cost per session against its $1,444 nominal EV — compressing its risk-adjusted edge back toward the designed "≈2× Moderate, at genuine career risk" shape. Exact weights/durations are playtest territory.
- **If post-content playtest still shows Aggressive dominating, tune event weights/durations first** — the §3 constants survived two calibration passes and should not churn for a costing knob that now lives in content.
